...
public class AjouterEvent    implements Initializable {
    ...
    @FXML
    private ImageView returnToAfficheevent;
    @FXML
    void returnToAfficheevent(MouseEvent event) {
        try {
    ...
    void uploadimageHandler(MouseEvent event) {
        ...
        if (file != null) {
            String fileName = file.getName(); // Obtenir juste le nom du fichier
            String targetDirectory = "C:\\Users\\msi\\Desktop\\elissa_travel+all+5_v0.5\\wala\\src\\main\\resources\\Event"; // Répertoire cible

            try {
                // Copier le fichier vers le dossier de destination avec le même nom
                Path sourcePath = file.toPath();
                Path destinationPath = Paths.get(targetDirectory, fileName);
                Files.copy(sourcePath, destinationPath, StandardCopyOption.REPLACE_EXISTING);

                // Mettre à jour le chemin du fichier affiché avec juste le nom du fichier
                file_path.setText(fileName);

                // Charger et afficher l'image depuis le nouveau chemin
                Image image = new Image("file:" + destinationPath, 500, 500, false, true);
